...
4. **Bloom filter** — removes items the user has already seen using a per-user Redis Bloom filter
5. **Feast item lookup** — resolves item features (category, price, gender) from a numpy in-memory cache loaded at startup (~0.1ms vs ~157ms for a live Feast round trip)
...
7. **DLRM ranking** — scores and re-orders the filtered candidates; results are returned to the caller enriched with DynamoDB item metadata
